            ...
            <div class="content">
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        $selectrecipe_id = mysqli_query($connect,"SELECT recipe_id FROM recipe_report GROUP BY recipe_id");
                        $counter = 1;
                        $countarray =array();
                        if(mysqli_num_rows($selectrecipe_id) !=0)
                        {
                            while($selectrecipe_idarray = mysqli_fetch_assoc($selectrecipe_id))
                            {
                                ...
                                $selectrecipecount = mysqli_query($connect,"SELECT user_first_name,user_last_name,user_info.user_id,recipe_info.recipe_id,recipe_name,count(recipe_report.recipe_id) FROM recipe_report INNER JOIN recipe_info ON recipe_info.recipe_id=recipe_report.recipe_id INNER JOIN user_info ON recipe_info.user_id=user_info.user_id WHERE recipe_report.recipe_id=$recipe_id_num ");
                            ...
                            usort($countarray, function($a, $b) {
                                if($a['count(recipe_report.recipe_id)'] == $b['count(recipe_report.recipe_id)'])
                                {
                                    return 0;
                                }
                                elseif($a['count(recipe_report.recipe_id)'] > $b['count(recipe_report.recipe_id)'])
                                {
                                    return -1;
                                }
                                elseif($a['count(recipe_report.recipe_id)'] < $b['count(recipe_report.recipe_id)'])
                             ...
                             {
                                    if($value['count(recipe_report.recipe_id)'] !=0)
                                    {
                                        echo '<tr>';
                                            echo '<td>'.$counter.'</td>';  
                                            ...
                                            echo '<td>'.$value['count(recipe_report.recipe_id)'].'</a></td>';
                                            echo '<td class="btn"><a href="removerecipe.php?recipe_id='.$value['recipe_id'].'">Remove</a></td>';
                                            echo '<td class="btn"><a href="deleterecipe.php?recipe_id='.$value['recipe_id'].'">Delete</a></td>';
                                        echo '</tr>';
                                        $counter++;
                                    }
                            }
